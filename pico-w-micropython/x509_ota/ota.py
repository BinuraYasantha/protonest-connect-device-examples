...
class OtaManager:
    ...
    def connect_mqtt(self):
        ...
        client = MQTTClient(
            ...
            ssl=True,
            ssl_params=self._build_mqtt_ssl_params(),
        )
    ...
    def _download_with_redirects(self, url, destination_path, redirect_count):
        ...
        try:
            ...
            request = (
                ...
                "Host: {}\r\n"
                "Connection: close\r\n"
                "User-Agent: protonest-pico-x509-ota/1.0\r\n"
                "\r\n"
            ).format(path, host)
    ...
    def _build_mqtt_ssl_params(self):
    ...
    def _create_https_ssl_context(self):
        return self._create_server_ssl_context(config.HTTPS_CA_DER_PATH)

    def _create_server_ssl_context(self, ca_path):
