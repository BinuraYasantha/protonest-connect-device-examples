...
class MQTTClient:
    ...
    def connect(self, clean_session=True, timeout=None):
        ...
        if self.user:
            size += 2 + len(self.user)
            msg[6] |= 0x80
            if self.pswd is not None:
                size += 2 + len(self.pswd)
                msg[6] |= 0x40
        ...
        index = 1
        while size > 0x7F:
            premsg[index] = (size & 0x7F) | 0x80
            size >>= 7
            index += 1
        premsg[index] = size

        self.sock.write(premsg, index + 2)
        self.sock.write(msg)
    ...
    def unsubscribe(self, topic):
        packet = bytearray(b"\xa2\0\0\0")
        self.pid += 1
        struct.pack_into("!BH", packet, 1, 2 + 2 + len(topic), self.pid)
        self.sock.write(packet)
        self._send_str(topic)
        ...
        while True:
            op = self.wait_msg()
            if op == 0xB0:
                response = self.sock.read(3)
                assert response[1] == packet[2] and response[2] == packet[3]
                return

    def wait_msg(self):
        result = self.sock.read(1)
