...
static const char *TAG = "protonest_x509_ota";
...
static void abort_pending_firmware_startup(const char *reason)
{
    if (!s_bootloader_rollback_pending) {
        return;
    }

    ESP_LOGE(TAG, "Startup failed while firmware is pending verification: %s", reason);
...
void app_main(void)
{
    ...
    ESP_LOGI(TAG, "Protonest Connect ESP-IDF X.509 OTA example");
    ESP_LOGI(TAG, "Transport: MQTT + HTTPS");
    ESP_LOGI(TAG, "Mode: X.509");
    ...
    if (!validate_device_identity()) {
        abort_pending_firmware_startup("device identity validation failed");
    ...
    ret = wifi_init_sta();
    if (ret != ESP_OK) {
        abort_pending_firmware_startup("wi-fi setup failed");
    ...
    ret = sync_clock();
    if (ret != ESP_OK) {
        abort_pending_firmware_startup("clock sync failed");
