...
static const char *TAG = "protonest_x509_publish";
...
{
    snprintf(buffer, buffer_size, "protonest/%s/stream/%s", DEVICE_NAME, stream_name);
}
...
void app_main(void)
{
    ...
    ESP_LOGI(TAG, "Protonest Connect ESP-IDF X.509 publish example");
    ...
    if (xTaskCreate(publisher_task, "protonest_publish", 4096, NULL, 5, NULL) != pdPASS) {
