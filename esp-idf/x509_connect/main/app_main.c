...
static const char *TAG = "protonest_x509_connect";
...
static void status_task(void *arg)
{
    while (true) {
        ...
        ESP_LOGI(TAG,
                 ...
                 (xEventGroupGetBits(s_wifi_event_group) & WIFI_CONNECTED_BIT) != 0 ? "connected" : "disconnected",
                 (xEventGroupGetBits(s_mqtt_event_group) & MQTT_CONNECTED_BIT) != 0 ? "connected" : "disconnected",
                 rssi,
                 DEVICE_NAME);

        vTaskDelay(pdMS_TO_TICKS(PROTONEST_STATUS_PRINT_INTERVAL_MS));
    }
}
...
void app_main(void)
{
    ...
    ESP_LOGI(TAG, "Protonest Connect ESP-IDF X.509 connect example");
    ...
    if (xTaskCreate(status_task, "protonest_status", 3072, NULL, 1, NULL) != pdPASS) {
